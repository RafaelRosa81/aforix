...
def _clean_analysis_pairs(analysis_pairs: pd.DataFrame | None) -> tuple[pd.DataFrame, list[dict[str, object]]]:
    ...
    if not out.empty:
        out = out.sort_values(
            ["station_id", "measurement_date", "analysis_group", "stage_origin", "stage_type"]
...
def _build_log(
    ...
    if "analysis_group" in matched.columns:
        for key, value in matched["analysis_group"].value_counts(dropna=False).items():
            rows.append({"metric": f"analysis_group.{key}", "value": int(value)})

    if "instrument" in matched.columns:
        for key, value in matched["instrument"].value_counts(dropna=False).items():
