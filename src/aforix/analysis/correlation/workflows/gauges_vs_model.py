...
def _coerce_date(value: str | None) -> pd.Timestamp | None:
    return None if not value else pd.to_datetime(value, format="%Y%m%d")


def _date_window(df: pd.DataFrame, start_date: pd.Timestamp | None, end_date: pd.Timestamp | None) -> pd.DataFrame:
    out = df
    if start_date is not None: out = out[out["date"] >= start_date]
...
def _role_columns(roles: dict[str, str]) -> tuple[str, str, str, str, str]:
    ...
    if roles["x"] not in role_to_col or roles["y"] not in role_to_col: raise ValueError(f"Invalid variable_roles for gauges_vs_model: {roles}")
    x_col = role_to_col[roles["x"]]; y_col = role_to_col[roles["y"]]; return x_col, y_col, f"q_{roles['y']}_pred_l/s", f"{roles['x'].capitalize()} q [l/s]", f"{roles['y'].capitalize()} q [l/s]"
...
def run_gauges_vs_model(*, normalized_root: Path, model_dir: Path, output_dir: Path, instruments: list[MeasuringInstrument], ranking_codes: list[str], start_date: str | None = None, end_date: str | None = None, points: list[str] | None = None, variable_roles: dict[str, str] | None = None) -> Path:
    ...
    for point in common_points:
        ...
        merged = merged.sort_values("date").reset_index(drop=True); x_values = merged[x_col].to_numpy(dtype=float); y_values = merged[y_col].to_numpy(dtype=float); slope, intercept, y_pred = _linear_fit(x_values, y_values); merged[pred_col] = y_pred; merged["residual_l/s"] = y_values - y_pred; merged["date_str"] = merged["date"].dt.strftime("%Y-%m-%d"); dmin = merged["date"].min().strftime("%Y%m%d"); dmax = merged["date"].max().strftime("%Y%m%d"); export = merged[["date_str", "q_gauge_l/s", "q_model_l/s", pred_col, "residual_l/s", "source"]].copy().rename(columns={"date_str": "time"}); export.to_csv(out_dir / f"{point}_gauge_vs_model_{dmin}_{dmax}.csv", index=False)
        n = len(merged); rmse_direct = rmse(y_values, x_values); rmse_reg = rmse(y_values, y_pred); q_mean_y = float(y_values.mean()) if n else float("nan"); row = {"Point": point, "X role": roles["x"], "Y role": roles["y"], "X variable": f"{roles['x']} [l/s]", "Y variable": f"{roles['y']} [l/s]", "X column": x_col, "Y column": y_col, "Linear equation": f"{roles['y']} = {slope:.6f} * {roles['x']} + {intercept:.6f}", "slope": slope, "intercept": intercept, "n": n, "R2": r2(y_values, y_pred), "Pearson r": pearson(x_values, y_values), "RMSE Y vs. X [l/s]": rmse_direct, "RMSE regression vs. Y [l/s]": rmse_reg, "q mean Y [l/s]": q_mean_y, "NRMSE Y vs. X [-]": rmse_direct / q_mean_y if q_mean_y else float("nan"), "MAE regression vs. Y [l/s]": mae(y_values, y_pred), "MAPE regression vs. Y [%]": mape(y_values, y_pred), "PBIAS regression vs. Y [%]": pbias(y_values, y_pred), "NSE regression vs. Y": nse(y_values, y_pred), "start": merged["date"].min().strftime("%Y-%m-%d"), "end": merged["date"].max().strftime("%Y-%m-%d"), "sources": " ".join(sorted(set(merged["source"].astype(str))))}; summary_rows.append(row)
        add_pair_sheet(wb, point, export, row, x_col=x_col, y_col=y_col, pred_col=pred_col, time_col="time", x_label=x_label, y_label=y_label); save_scatter_with_regression(export, x_col=x_col, y_col=y_col, pred_col=pred_col, x_label=x_label, y_label=y_label, out_path=plots_dir / f"{point}_scatter_gauge_vs_model.png"); save_time_series(export, time_col="time", series=[(x_col, roles["x"].capitalize()), (y_col, roles["y"].capitalize())], out_path=plots_dir / f"{point}_timeseries_gauge_vs_model.png")
    if summary_rows:
        a = start.strftime("%Y%m%d") if start is not None else "NA"; b = end.strftime("%Y%m%d") if end is not None else "NA"; pd.DataFrame(summary_rows).to_csv(out_dir / f"summary_gauges_vs_model_{ranking_label}_{a}_{b}.csv", index=False); write_summary_sheet(wb, "SummaryMetrics", summary_rows); safe_save_workbook(wb, out_dir / f"correlation_gauges_vs_model_{ranking_label}_{a}_{b}.xlsx")
    return out_dir
