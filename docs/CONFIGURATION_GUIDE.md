...
Durante la ingesta, Aforix espera poder obtener `station_id`, `station_name`, fecha y hora de inicio, caudal total, área total, ancho total, velocidad media, profundidad media y temperatura si está disponible.
...
La hoja esperada por defecto es `Datos`, si así está definido en la configuración.
...
Los archivos en `configs/normalization/` definen cómo convertir columnas crudas o raw canonical a columnas canónicas.
